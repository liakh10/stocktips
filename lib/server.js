...
/* A private endpoint (dRPC) is the last resort here, on the server, only from an env var, after the public RPCs.
   It must never reach lib/chains.js: that file ships to the browser and the token would be public. */
const RPC = ['https://rpc.mainnet.chain.robinhood.com', 'https://robinhood-rpc.publicnode.com', process.env.ROBINHOOD_RPC].filter(Boolean);
